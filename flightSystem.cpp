...
int flightMatrix::addCity(string cityName){
    ...
    if (searchCity(cityName) == -1){
        ...
        //cout << "Error: City already exists.\n";
        return -1;
    }
...
//        newVertex.from_id = -1;
    ...
    for (iter = vertices.begin(); iter != vertices.end(); iter++){
        ...
        for( vFlightIt = outboundFlights.begin(); vFlightIt!= outboundFlights.end(); vFlightIt++){
            ...
            //cout << (*vFlightIt).departure_city << "  " << (*vFlightIt).destination_city << "   " << (*vFlightIt).cost << endl;
            for(iter2 = vertices.begin(); iter2 != vertices.end(); iter2++){
            
                if((*iter2).id == searchCity((*vFlightIt).destination_city)){
                    
                    if(!(*iter).route.empty()){
                        arrival_time = (*iter2).route.back().arrival_time;
                    }else{
                    ...
                    if((((*iter2).cost > ((*vFlightIt).cost + (*iter).cost)) || ((*iter2).cost == -1)) && (arrival_time <= (*vFlightIt).departure_time)){
                        ...
                        //cout << lookupCity((*iter).id) << "  " << (*vFlightIt).destination_city << "  " << (*iter2).cost << " " << (*vFlightIt).cost << " " << routeCost << endl;
                    
/*--->*/            
                    //cout << (*iter2).route.size();

                    //(*iter2).route.push_back(*vFlightIt);
                    //cout << (*iter2).route.back().flight_id << endl;
    ...
    for(iter2 = vertices.begin(); iter2 < vertices.end(); iter2++){
        //cout << lookupCity((*iter2).id) << " " << (*iter2).cost << " " << (*iter2).route.size() << endl;
        if(lookupCity((*iter2).id) == destination_city){
                ...
                cout << "============== " << "=========== " << "========== " << "============== " << "============ " << "======== " << "========" << endl;
                //cout << departure_city << " " << destination_city << " " << (*iter2).cost << " " << endl;
